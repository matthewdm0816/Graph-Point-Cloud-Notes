...
具体的VQA模型使用的是co-attention model\footnote{
    Zhou Yu, Jun Yu, Chenchao Xiang, Jianping Fan, andDacheng Tao.  Beyond bilinear: Generalized multimodal factorized high-order pooling for visual question answering, 2018.
}
...
\end{equation}
之后接上dropout, power norm.$z \leftarrow \operatorname{sign}(z)|z|^{0.5}$
...
\begin{remark}
    说实在的, marginal improvement on VQA. (0.7\% on old models), 作者也指出了在Image Captioning上效果比较好(2-3\%)
\end{remark}

\section{MCAN: Deep Modular Co-Attention Networks for VQA}

本工作设计了一种好的co-attention机制来做VQA任务的特征融合. 最近的工作展示了co-attention可以更好的学习image-text repr., 但是缺乏image region和word的关联. 为了解决这些问题, BAN, DCN作为dense co-att模型. 他们都可以叠成多层co-att, 然而性能并没有提高多少! 我们认为问题在于在后面的层里存在不少自己模态内的注意力. 使用SA(SelfAtt)模块建模intra-modality att和GA(Guided Att)模块来建模inter-modality att. 通过结合它们,得到了MCA层.

\begin{remark}
    Commonsense推理(如同VC RCNN中)是否在coattention/correlation inference里make sense?
\end{remark}

\subsection{Modular Co-Attention Layer}

\cfig{mcan-mod.png}

使用不同的组合, 得到三种不同的MCA.
\subsubsection{SA \& GA Units}

\cfig{mcan-var.png}

简单叙述Attention机制, 给定
$\bs q \in \R^{1\times d}, \bs K, \bs V \in \R^{N\times d}$,
得到的按注意力加权feature
\begin{equation}
    f=A(q, K, V)=\operatorname{softmax}\left(\frac{q K^{T}}{\sqrt{d}}\right) V
\end{equation}
Multi-head Attention涉及使用多个类似的模块
\begin{equation}
    \begin{array}{c}
    f=M A(q, K, V)=\left[\text { head }_{1}, \text { head }_{2}, \ldots, \text { head }_{h}\right] W^{o} \\
    \qquad{h e a d}_{j}=A\left(q W_{j}^{Q}, K W_{j}^{K}, V W_{j}^{V}\right)
    \end{array}
\end{equation}
其中\begin{equation}
    W_{j}^{Q}, W_{j}^{K}, W_{j}^{V} \in \mathbb{R}^{d \times d_{h}}
\end{equation}是第j个head的投影矩阵, 并且
\begin{equation}
    W_{o} \in \mathbb{R}^{h * d_{h} \times d}
\end{equation}
为了防止MHA参数过多, 使用$d_h=d/h$.

具体上, SA的MLP层为(FC(4d)-ReLU-Dropout(0.1)-FC(d)). 接着加上resiudal link和layer norm..

\cfig{mcan-arch.png}

\subsection{Modular Co-Attention Networks}

我们提出stacking+encoder/decoder模型, 包含了多个MCA层.  

\cfig{mcan-arch2.png}

使用的视觉特征是从Faster RCNN(VG上pre-trained)中提取的各层特征, 使用Bottom-Up feature. 使用10-100个目标区域(取决于confidence, w/ threshold), $X \in \mathbb{R}^{m \times d_{x}}$. 

问题token化, 转换成最多14个词, 每个word使用300d GloVe(pretrained) embed. 词embed送到单层LSTM中, 并且使用整个输出作为问题的特征矩阵$Y \in \mathbb{R}^{n \times d_{y}}$.

为了处理可变长长度, 使用0-padding来把填充到最长.(i.e.,m=100, n=14). 每个softmax之前把padding logits改成$\infty$.

具体地堆叠形式请看图.

最后, 加上attetional reduction(FC(d)-ReLU-Dropout(0.1)-FC(1)). 
\begin{equation}
    \begin{array}{l}
    \alpha=\operatorname{softmax}\left(\operatorname{MLP}\left(X^{(L)}\right)\right) \\
    \tilde{x}=\sum_{i=1}^{m} \alpha_{i} x_{i}^{(L)}
    \end{array}
\end{equation}
然后融合
\begin{equation}
...
最后, z投影到N分类向量上作为分类结果, 并且使用BCE来训练N-classifier.

\subsection{Experiments}

最后使用$MCAN_{ed}-6$作为best single model, 精度70.63/70.90 (test-dev/std), 在train+val+vg上训练!(vg是从VG数据集来的增强VQA样本).

通过ensemble, 精度可以达到72.45.

\section{TRRNet:Tiered Relation Reasoning for Compositional VQA}

现在SoTA VQA方法可以分为两类: 第一类同时学习multi-modal joint表示, 常常使用多层注意力来fuse V+L repr., 这些方法性在compositional VQA上效果不好. 第二类是设计特别的NNs来进行不同任务上的reasoning, 他们可以在CLEVR上效果极佳, 但是很tricky, 实际数据集上难以利用. 

本工作提出tiered rel. reasoning来动态选择obj. lvl. 候选, 根据lan. repr., 并且只在选定的obj. 之间学习关系. 本作提出的TRRNet包含TRR单元, 每个包含: root att., root$\to$ leaf att. passing, leaf att., final message passing mod. 

\subsection{TRRNet}

\subsubsection{Overview}

\cfig{trrnet-arch.png}

图片经过OD, 提取出n个region vis. feat.$V \in \mathbb{R}^{n \times d_{v}}$
, 和 bounding box$B \in \mathbb{R}^{n \times d_{b}}$.
语言经过BERT的词嵌入, 并且送进GRU来得到更好的语言embedding $E \in \mathbb{R}^{m \times d}_{e}$

\subsection{Root Attention}

obj. lvl. att.
\begin{equation}
    \alpha^{\text {object }}=\operatorname{softmax}(\operatorname{Net}(V, B, E))
\end{equation}
apply att., 得到融合的vis. feat.
\begin{equation}
    O^{\text {root }}=\alpha^{\text {object }} V^{T} \text { , }
\end{equation}

有很多root att.的选择, like in Bottom-Up, DCN, BAN, Inter-Intra. 本作使用Bottom-Up和BAN里的方法.

\subsection{Root to Leaf Attention Passing}

使用multi-head \bt{hard} att. 来选择rel. obj.候选. 但是这样的hard att. 不可微分. REINFORCE可以用来通过sampling来得到网络参数(Xu et al., ICML '15). 本文使用一种更简单和完全可微的方式.
...
也可以使用refined obj. 候选的triplet relation, 得到关系
$R_{\text {triple }} \in \mathbb{R}^{K^{3} \times d_{r}}$

\subsubsection{Leaf Attention}

和root att.相同, 使用关系表示$R \in \mathbb{R}^{K^{2} \times d_{r}}$
...
\subsubsection{Message Passing module for units interaction}

为了可以进行multi-stage reasoning, 提出message passing module, 
\begin{equation}
    V_{n e w}=f\left(\left[O^{l e a f}, V\right]\right)
\end{equation}

\subsubsection{Multi-stage Reasoning and Policy Network}

形式上, 一层TRR unit输入bounding box feat. B, t-th vis. feat. $V_t$, question embed. E, 输入$O_{t}^{\text {root }}, O_{t}^{\text {leaf }}, V_{t+1}$ 
\begin{equation}
    O_{t}^{\text {root }}, O_{t}^{\text {leaf }}, V_{t+1}=T R R_{t}\left(B, V_{t}, E\right)
\end{equation}

在第t个TRR, policy net., 若和上一步的到底哦attended feat.没有区别, 则停止继续推理.
\begin{equation}
    \begin{array}{c}
    ...
    \end{array}
\end{equation}  
policy net.使用REINFORCE中的policy gradient方法来训练. 在训练中分别地训练两个网络(同时保持另一个网络参数固定, 并且最多三步), policy net. 的loss是
\begin{equation}
    L=-E_{s \sim \pi}[r-p]
\end{equation}
其中每走一步penalty为0.1.

\subsubsection{The Readout Layer}
\begin{equation}
...
\section{Weekly Seminar 4.28}

\subsection{Causal Signal on Images}

图像不同类别是否有不对称的统计量? obj. feat. / context feat.

使用判别器来拟合样本之间的因果关系.

提出NCC(!)
\begin{equation}
    \operatorname{NCC}\left(\left\{\left(x_{i j}, y_{i j}\right)\right\}_{j=1}^{m_{i}}\right)=\psi\left(\frac{1}{m_{i}} \sum_{j=1}^{m_{i}} \phi\left(x_{i j}, y_{i j}\right)\right)
\end{equation}

\subsection{Causual View on Robustness}

考虑三种导致输出X的r.v. Y(label), Z, M(latent causes, 可人工干预)

\begin{equation}
    p_{\theta}(x, y, z, m)=p(m) p(z) p(y) p_{\theta}(x \mid y, z, m) .
\end{equation}

给出一个变分框架建模隐变量.

\subsection{Calibrated Asymmetric Surrogate Loss}


\section{GCNII: GCN via Initial residual and Identity Mapping}

\subsection{Related Models}

\bt{APPNP} 使用了PPR(Personalized Page-Rank)矩阵来导出K-order的滤波器
\begin{equation}
    \mathbf{H}=\alpha\left(\mathbf{I}_{n}-(1-\alpha) \tilde{\mathbf{A}}\right)^{-1} f_{\theta}(\mathbf{X})
\end{equation}

To make k-hop aggr., 近似的有Taylor展开
\begin{equation}
    \boldsymbol{H}^{(\ell+1)}=(1-\alpha) \tilde{\boldsymbol{P}} \boldsymbol{H}^{(\ell)}+\alpha \boldsymbol{H}^{(0)}
\end{equation}

\bt{JKNet}是第一个深度GCN网络. 在最后一层, JKNet使用了之前所有层的表示的Concat来从不同层级学到表示. 根据他们的证明, 这能解决over-smoothing的问题

\bt{DropEdge}建议随机地drop edge来降低在over-smoothing上的收敛. 一个典型的网络层为
\begin{equation}
    \mathbf{H}^{(\ell+1)}=\sigma\left(\tilde{\mathbf{P}}_{\text {drop }} \mathbf{H}^{(\ell)} \mathbf{W}^{(\ell)}\right)
\end{equation}

\subsection{GCNII}
想法是不适用ChebNet/GCN那样的固定系数, 而是学到任意系数的多项式.
\begin{equation}
...
\end{equation}
类似APPNP, 这里第一项试图在学到至少$\alpha_{\ell}$那么多的原特征. 实践中常常设置为0.1或者0.2.

类似ResNet,第二项借用了恒等映射.恒等映射被证明具有一下性质, 一个线性ResNet满足
\begin{enumerate}
    \item 最优的权值矩阵具有小的范数
    \item 唯一的临界点是全局最小点
\end{enumerate}

(Oono \& Suzuki, 2020)证明了对于K-层的GCN的节点特征将会收敛到一个子空间, 导致info loss. 收敛速度取决于$s^K$, 是权值矩阵的最大奇异值. 使用$\left(1-\beta_{\ell}\right) \mathbf{I}_{n}+\beta_{\ell} \mathbf{W}^{(\ell)}$可以将奇异值拉近1, 防止特征空间退化.

设置$\beta_{\ell}$的原则是根据不同的层改变, 实验中使用的是$\beta_{\ell}=\log \left(\frac{\lambda}{\ell}+1\right) \approx \frac{\lambda}{\ell}$

\subsection{Connection to Iterative Shrinkage-Thresholding}

最近有一些基于优化观点的网络结构设计工作. FFN被看作是对一些目标函数的迭代优化, 一个假设是更好的优化算法lead to更好的结构. 考虑LASSO目标函数
\begin{equation}
    \min _{x \in \mathcal{R}^{n}} \frac{1}{2}\|\mathbf{B} \mathbf{x}-\mathbf{y}\|_{2}^{2}+\lambda\|\mathbf{x}\|_{1}
\end{equation}
Iterative-shrinkage alg.可以有效的解决上述优化问题, 他的迭代为
\begin{equation}
    \mathbf{x}^{t+1}=P_{\mu_{t} \lambda}\left(\mathbf{x}^{t}-\mu_{t} \mathbf{B}^{T} \mathbf{B} \mathbf{x}^{t}+\mu_{t} \mathbf{B}^{T} \mathbf{y}\right)
\end{equation}
其中$\mu_t$是迭代步长, 
\begin{equation}
    P_{\theta}(z)=\left\{\begin{array}{lr}
    z-\theta, & \text { if } z \geq \theta \\
    0, & \text { if }|z|<\theta \\
    z+\theta, & \text { if } z \leq-\theta
    \end{array}\right.
\end{equation}
是soft-thresholding函数.

\subsection{Spectral Analysis}

\begin{theorem}
    Assume the self-looped graph $\tilde{G}$ is connected. Let $\mathbf{h}^{(K)}=\left(\frac{\mathbf{I}_{n}+\tilde{\mathbf{D}}^{-1 / 2} \tilde{\mathbf{A}} \tilde{\mathbf{D}}^{-1 / 2}}{2}\right)^{K} \cdot \mathbf{x}$ denote the representation by applying a $K$ -layer renormalized graph convolution with residual connection to a graph signal $\mathrm{x}$. Let $\lambda_{\tilde{G}}$ denote the spectral gap of the self-looped graph $\tilde{G}$, that is, the least nonzero eigenvalue of the normalized Laplacian $\tilde{\mathbf{L}}=\mathbf{I}_{n}-\tilde{\mathbf{D}}^{-1 / 2} \tilde{\mathbf{A}} \tilde{\mathbf{D}}^{-1 / 2} .$ We have
    1) As $K$ goes to infinity, $\mathbf{h}^{(K)}$ converges to $\boldsymbol{\pi}=\frac{\left\langle\mathbf{D}^{1 / 2} \mathbf{1}, \mathbf{x}\right\rangle}{2 m+n}$.
    $\tilde{\mathbf{D}}^{1 / 2} \mathbf{1}$, where $\mathbf{1}$ denotes an all-one vector.
...
猜想: 更高degree的节点特征会更容易over-smoothed.

\begin{theorem}
    Consider the self-looped graph $\tilde{G}$ and a graph signal x. A $K$ -layer GCNII can express a $K$ order polynomial filter $\left(\sum_{\ell=0}^{K} \theta_{\ell} \tilde{\mathbf{L}}^{\ell}\right) \mathbf{x}$ with arbitrary coefficients $\theta .$
\end{theorem}

这保证了GCNII的expressiveness. 并且在层数趋近无穷的时候特征分布不会退化.

\section{MLPMixer}

\cfig{mlpmixer-arch.png}

主要包含了两种MLP, token-mixing和channel-mixing MLP
\begin{equation}
    \begin{aligned}
    \mathbf{U}_{*, i} &=\mathbf{X}_{*, i}+\mathbf{W}_{2} \sigma\left(\mathbf{W}_{1} \text { LayerNorm }(\mathbf{X})_{*, i}\right), & & \text { for } i=1 \ldots C, \\
    \mathbf{Y}_{j, *} &=\mathbf{U}_{j, *}+\mathbf{W}_{4} \sigma\left(\mathbf{W}_{3} \text { LayerNorm }(\mathbf{U})_{j, *}\right), & & \text { for } j=1 \ldots S .
    \end{aligned}
\end{equation}


\section{DETR:E2E Object Detection with Transformers}

现代的Object Detectors一般使用一些代理任务来进行物体检测(by proposals/anchors/window centers). 为了简化pipeline, 他们设计了一个E2E的物体检测方法.

\cfig{detr-arch.png}
在CNN后接着一个Transformer的encoder-decoder结构, 来直接得到prediction box, 最后使用bipartite loss来和GT进行matching. 同时DETR的输出无需NMS之类的后处理.

\subsection{Set Prediction Loss}

DETR预测一个固定长度的N个物体, 使用bipartite matching来得到loss
\begin{equation}
...
\end{equation}
其中$\sigma$是一个permutation. 使用匈牙利算法可以有效地计算最优的alignment. 并且定义matching cost为
\begin{equation}
...
\end{equation}
实践中, 对于空类将其权重变为0.1, 来解决class imbalance, 和Faster RCNN相同.
注意在Matching Loss里我们使用的是prob而不是log-prob, 为了和box-loss有相同的量级.

关于BB Loss, $\ell1$范数对不同大小的BB有不同的权重, 所以使用$\ell1$范数和广义IoU的组合:
\begin{equation}
...
使用auxiliary decoding loss来帮助训练decoder.在每层decoder后面加入LN+FFN和Hugarian Loss(FFN共享权值), 来作为辅助decoder.

\section{Counterfactual VQA: A Cause-Effect Look at Language Bias}
\cfig{cfvqa-arch.png}
VQA数据集中常常具有巨大的language bias. 比如, 在所有'Can you see a...'问题中回答yes就有90\%的准确率!

最近,提出了VQA-CP数据集, trainset/testset具有不同的语言先验, 这要求模型必须减少language bias. 为了解决这个问题, 最近的方法包括加强Visual Grounding, 弱化语言先验,和implicit/explicit数据增强. \begin{enumerate}
    \item 使用人类的视觉/文本解释来强化visual grounding.
    \item 使用ensemble多个QA分支来capture 语言先验, 通过对抗学习/多人物学习
    \item 自动生成额外的QA对来balance data.
\end{enumerate}
\subsection{Preliminaries}
